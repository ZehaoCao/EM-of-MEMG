...
mpc.bus = [ %% (Pd and Qd are specified in kW & kVAr here, converted to MW & MVAr below)
    ...
    30	1	200	400	0	0	1	1	0	12.66	1	1.1	0.9;
...
mpc.branch = [  %% (r and x specified in ohms here, converted to kohms below)
	...
	32	33	0.3410	0.5302	0	0	0	0	0	0	1	-360	360;    %% as a radial network
];
...
%mpc.bus(:, [PD, QD]) = mpc.bus(:, [PD, QD]) / 10e3;
%% adjust parameters
load_factor = 1;
mpc.bus(:, [PD, QD]) = mpc.bus(:, [PD, QD]) *10;%to KW
mpc.bus(:,QD) = mpc.bus(:, QD) *1.15;%to KW
mpc.bus(:,PD) = mpc.bus(:, PD) *1;%to KW

mpc.bus(:,QD) = mpc.bus(:, QD) *1.5*load_factor;%to KW
mpc.bus(:,PD) = mpc.bus(:, PD) *1.5*load_factor;%to KW

mpc.bus(:, [PD, QD]) = mpc.bus(:, [PD, QD]) /1e3;%to MW
mpc.branch(:, [BR_R BR_X]) = mpc.branch(:, [BR_R BR_X])/7.5;% r and x specified in ohms here
mpc.branch(:, [BR_R BR_X]) = mpc.branch(:, [BR_R BR_X])/1e6;% r and x are converted to Mohms

%mpc.branch(:, [BR_R BR_X]) = mpc.branch(:, [BR_R BR_X])*10;

mpc.branch(:, [BR_R BR_X]) = mpc.branch(:, [BR_R BR_X])/(1.5*load_factor);% r and x are converted to Mohms
